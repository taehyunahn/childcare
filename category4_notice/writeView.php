...
<body>
    <?php       
        $bno = $_GET['seq'];
        $sql1="SELECT * FROM notice WHERE seq ='".$bno."'";
        $result1 = mysqli_query($conn,$sql1);
        $hit = mysqli_fetch_array($result1);
        $hit = $hit['hit'] + 1;
        
        $sql2="UPDATE notice SET hit = '".$hit."' where seq = '".$bno."'";
        $result2 = mysqli_query($conn,$sql2);
        
        $sql3="SELECT * FROM notice WHERE seq='".$bno."'";
        $result3 = mysqli_query($conn,$sql3);
        $board = $result3->fetch_array();

        // $hit = mysqli_fetch_array(mq("select * from notice where seq ='".$bno."'"));
        // $hit = $hit['hit'] + 1;

        // $fet = mq("update notice set hit = '".$hit."' where seq = '".$bno."'");

        // $sql = mq("select * from notice where seq='".$bno."'"); /* 받아온 idx값을 선택 */
        // $board = $sql->fetch_array();

        
        //칼럼번호가 데이터베이스에 없다면,
        $sql5="SELECT EXISTS (SELECT * FROM notice WHERE seq = $bno) as isChk";
            ...
            <div class="container">
                ...
                <main>
                    ...
                    <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                        <!-- 버튼1. 목록 (비로그인도 확인 가능) -->
                        <a class="btn btn-outline-dark  me-md-1" href="/category4_notice/noticeMain.php"
                            role="button">목록</a>
                        <!-- 버튼2. 수정 (로그인 후 확인 가능) -->
                        <?php
                            if($_SESSION['name']==$board['writer']||$_SESSION['role']=='admin'){
                                echo "<a class=\"btn btn-outline-dark\" href=\"/category4_notice/writeEdit.php?seq=".$_GET['seq']."\" role=\"button\">수정</a>";
                                } else {
                                }
                                ?>
