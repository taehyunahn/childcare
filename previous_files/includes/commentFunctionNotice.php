...
function setComments($conn) {
    if (isset($_POST['commentSubmit'])) {
        ...
        $sql = "INSERT INTO commentNotice (name, date, content, con_num) VALUES ('$name', '$date', '$content', '$con_num')";
        $result = $conn -> query($sql);

        $sql11 = "UPDATE notice SET comment= comment +1 WHERE seq='$con_num'";
        $result11 = $conn -> query($sql11);

        echo '<script> alert("댓글을 입력했습니다."); location.href="/category4_notice/writeView.php?seq='.$con_num.'";</script>';
...
function getComments($conn) {
    ...
    $sql = "SELECT * FROM commentNotice WHERE con_num = '$con_num'";
    $result = $conn->query($sql);
...
";
        ...
        if($_SESSION['name']==$row['name']){
            ...
            echo "
            <form class=\"p-2 bd-highlight\" method='POST' action='/category4_notice/commentEdit.php'>
        ...
        <form class=\"p-2 bd-highlight\" method='POST' action='/category4_notice/commentDelete.php'>
            <input type = 'hidden' name='seq' value = '".$row['seq']."'>
            <input type = 'hidden' name='con_num' value = '".$con_num."'>
...
function editComments($conn) {
    if (isset($_POST['commentSubmit'])) {
        ...
        $sql = "UPDATE commentNotice SET content='$content' WHERE seq='$seq'";
        $result = $conn -> query($sql);
        // header("Location: /category3_review/view.php?seq='".$con_num."'");
        header("Location: /category4_notice/writeView.php?seq=$con_num");
